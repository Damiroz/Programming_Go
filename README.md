# Практическое занятие №13 Ибраимов Дамир ПИМО-01-25
...
1. Текущее потребление: Профиль зафиксировал ~1.5 MB удерживаемой памяти.
2. Основные источники аллокаций:
runtime.allocm (1026 kB / 66.71%): выделение памяти под системные потоки (M) для работы планировщика Go.
runtime.(*timers).addHeap (512.01 kB / 33.29%): память, занятая внутренними таймерами среды выполнения.
...
|---------|----------------|-------------------|-----------|
| Время выполнения (ns/op) | 3,863,825 ns/op | 16.71 ns/op | в в 231 228 раз |
| Количество операций в бенчмарке | 304 | 60,976,405 |  |
| Потребление памяти (B/op) | 0 B | 0 B | без изменений |
| Аллокации (allocs/op) | 0 | 0 | без изменений |
| Время HTTP-запроса | ~2.16 секунды | ~0.000017 секунды | в 231 228 раз |
...
- Время выполнения (ns/op): сократилось с 3 863 825 нс (~3.86 мс) до 16.71 нс на одну операцию.
- Пропускная способность: количество итераций, выполняемых за цикл теста, выросло с 304 до 60 976 405.
- Эффективность памяти: оба алгоритма сохранили показатель 0 B/op, что подтверждает отсутствие лишних аллокаций в куче (Heap).
