### Практика 16. Ибраимов Дамир ПИМО-01-25. Интеграционное тестирование API. Использование Docker для тестовой БД
...
```
![screen](./screens/image1.png)
### Запуски тестов/результаты
```
go test -v ./internal/integration/notes_integration_test.go 
```
1. POST и GET записи
2. GET несуществующей записи
3. обновление записи через PUT
4. DELETE записи
5. Пагинация записей
![screen](./screens/image2.png)

```
docker compose ps 
```
![screen](./screens/image3.png)


### Заключение по итогам интеграционного тестирования:

В ходе работы реализовано полноцикловое интеграционное тестирование сущности «Заметка» (Note). Тесты охватывают все уровни приложения — от обработки HTTP-запросов (слой Handler) до выполнения SQL-запросов в изолированном контейнере PostgreSQL (слой Repository).

Ключевые результаты:
Верификация CRUD-операций: Подтверждена корректность работы методов Create, Read, Update, Delete. Проверена сквозная передача данных между слоями приложения.

Валидация и краевые сценарии: * Реализована проверка обработки несуществующих записей (возврат HTTP 404).

Протестирована логика валидации входящих данных (возврат HTTP 400 при пустых полях).

Управление списками: Проверена корректность механизма пагинации (параметры limit и offset), что гарантирует стабильную работу API при больших объемах данных.

Инфраструктурная изоляция: Тестирование проведено в среде Docker, что обеспечило полную идентичность тестового окружения и отсутствие конфликтов с локальными базами данных.
