### Практика 15. Ибраимов Дамир ПИМО-01-25. Unit-тестирование функций (testing, testify)
...
1. Объект тестирования: Реализовано модульное тестирование для арифметических функций (mathx), манипуляций со строками (stringsx) и бизнес-логики (service).

2. Примененные подходы: В ходе работы освоены табличные тесты для проверки наборов данных и метод t.Run для логической группировки кейсов.

3. Изоляция зависимостей: Ключевым этапом стала проверка сервисного слоя с помощью тест-даблов (stub), что позволило протестировать логику в полной изоляции от базы данных через интерфейсы.

4. Покрытие сценариев: Особое внимание уделено негативным кейсам (деление на ноль, пустые строки) и граничным условиям, что обеспечило надежность кода при некорректных входных данных.

5. Инструментарий: Использование библиотеки testify (пакеты assert и require) позволило сделать тесты более читаемыми и упростило проверку возвращаемых ошибок .
